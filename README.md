...
- Runtime helpers including `ref`, read-only and writable `computed`, `effect` with optional scheduling, `watch`, `watchEffect` with cleanup callbacks, `nextTick`, lifecycle callbacks including KeepAlive activation hooks, `provide`, `inject`, and `defineAsyncComponent` with ErrorBoundary handoff
