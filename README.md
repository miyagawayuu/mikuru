...
- Runtime helpers including `ref`, read-only and writable `computed`, `effect`, `watch` with `immediate`, `once`, and cleanup callbacks, `nextTick`, lifecycle callbacks including KeepAlive activation hooks, `provide`, `inject`, and `defineAsyncComponent` with ErrorBoundary handoff
