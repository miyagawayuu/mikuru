...
- SSR through `compileSsr()` and `mikuru/server`, covering escaped text, static and bound attributes, `v-if` chains, `v-for`, async child components, props, named/default slots, scoped slot props, Teleport collection, and router route rendering
- Hydration through `compileHydration()` and `hydrateRoute()`, reusing existing SSR DOM while attaching events, syncing text/attributes, hydrating initial `v-if` / `v-for` DOM, Teleport target content, and delegating child and route components to `hydrate()` when available
