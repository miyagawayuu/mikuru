...
- The Vite plugin supports `mikuru({ debug: true })`, which appends a generated `sourceURL` comment to transformed `.mikuru` modules and enables the stable v1 `globalThis.__MIKURU_DEVTOOLS__` component metadata and debug event hook. `createDevtoolsInspector()` can read flat components, component trees, event history, events by type, and a versioned snapshot, and can subscribe to that hook. Debug events include `version: 1`, component lifecycle/update/error events, style injection events with style id/scope metadata, async events, router events, `v-*` compatibility warnings, and `hydration:warning` events with phase, component, filename, message, kind, action, expected/actual, and DOM path payloads where available.
- The compiler returns a v3 source map with `file`, `sources`, `sourcesContent`, `names`, generated-line mappings, and generated-column segments for common template elements, interpolations, bound attributes, event handlers, script lines, and style injection. Vite forwards that map to the bundler.
- Generated JavaScript can still be inspected when a compiler issue needs exact generated-code context beyond the supported segment mappings.
