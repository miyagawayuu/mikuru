...
- `hydrateRoute(router, target, location?, options?)` resolves redirects, lazy route components, route props, route component context, and nested route components, then hydrates matched route components with `hydrate()` or mount fallback. `{ listen: true }` starts router history listening after hydration and stops it on unmount; `examples/router-ssr-hydration` keeps route SSR and route hydration wired together for browser E2E coverage.
