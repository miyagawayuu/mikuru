...
- `<style scoped>` supports selector rewriting for common selectors, escaped selectors, attribute selectors, functional pseudo-classes, native CSS nesting, nested at-rules including `@scope`, unknown at-rules with nested rules, `:global(...)`, and `:deep(...)`; helper-looking text inside attributes and functional pseudo-class arguments is preserved, malformed block diagnostics include offset/line/column, and raw blocks such as `@keyframes`, `@font-face`, and `@property` are preserved.
