...
- `ref`、`isRef`、`unref`、`toRef`、`toRefs`、`reactive`、`readonly`、`computed`、`effect`、`queueJob`、`flushJobs`、`watch`、`watchEffect` は `mikuru` または `mikuru/runtime` からimportする。`computed` は `computed(() => value)` と `computed({ get, set })` の両方に対応する。
...
- `@media`、`@supports`、`@container`、`@layer`、`@scope` の中のルールも書き換える。未知のat-ruleでも中にCSSルールがあれば、その内側のセレクタをscope対象にする。
...
- 閉じ括弧が足りないCSSブロックは元のCSSを保持し、debug時はstyle diagnosticの対象になる。
...
<button @click.right="openMenu" @click.ctrl.exact="openExact">Open</button>
